...
2. 纯扫描 / 预演 / diff 过程
3. 自动同步链路
4. 备份链路（手动备份 / 自动备份 / 切换备份 / 初始化上传，含云端1、云端2、本地）

说明：

- merge 的破坏性和目标定义与 restore / revert 不完全一致，先不混进第一版。
- 第一版先把最危险的 restore / revert 做稳。
- 备份链路属于“副本写入（best-effort）”，不会修改当前浏览器书签树本体。
- 备份链路中断的主要影响是“本次备份可能不完整或未落盘”，可通过下一次备份自然收敛。
- 因此 v1 不为备份链路引入“强制中断恢复面板”，以避免额外常驻判断与网络开销。
...
- 备份链路（手动/自动/切换/初始化）不纳入中断恢复面板。
- 理由是：备份链路不会破坏浏览器当前书签；中断不会把用户留在“书签被清空/半恢复”的危险状态。
- 覆盖模式下，允许存在“小概率本次覆盖备份未完整写入”的风险；这属于可接受的小风险。
- 若发生中断，按“下次重试备份”处理，不增加强制阻断 UI。
...
9. **备份链路（云端/本地上传下载）不纳入中断恢复面板，按 best-effort + 下次重试处理。**
