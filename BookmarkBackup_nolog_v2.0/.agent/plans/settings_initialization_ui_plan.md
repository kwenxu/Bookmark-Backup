...
- **Simple**（简略）：只显示变化摘要统计
- **Detail**（详细）：显示具体的变化列表（带 `[+][-][~][↔]` 符号），**包含完整变更数据，未来可由此推导出 Simple 模式的统计信息**
...
> 📋 详细计划书：[complete_restore_wizard_plan.md](./complete_restore_wizard_plan.md)

**功能定位**：在「设置与初始化」面板的右侧区域，提供完整的恢复向导
...
│  ┌─ 左侧：设置区域 ────────┐  ┌─ 右侧：恢复向导入口 ─────────────┐  │
│  │  Bookmark Backup        │  │                                  │  │
│  │  ├─ Mode: Auto/Manual   │  │   📦 完整恢复向导                 │  │
│  │  ├─ Time: [⚙️ 设置]     │  │   从云端或本地备份恢复书签和历史   │  │
│  │  └─ Overwrite: ...      │  │                                  │  │
│  │                         │  │   [开始恢复 →]                    │  │
│  │  Backup History         │  │                                  │  │
│  │  └─ ...                 │  │   ─────────────────────────────  │  │
│  │                         │  │   最近云端备份:                   │  │
│  │  初始化操作             │  │   • WebDAV: 2026-01-09 15:30     │  │
│  │  └─ [重置] [初始化上传]  │  │   • GitHub: 2026-01-09 14:20     │  │
│  └─────────────────────────┘  └──────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────┘
```
...
| 功能 | 说明 | 优先级 |
|------|------|--------|
| **完整恢复向导** | 引导用户按正确顺序恢复书签和历史 | 🔴 高 |
| **智能检测** | 检测当前书签与导入历史的匹配度 | 🔴 高 |
| **云端检测** | 自动检测云端是否有可用备份 | 🟡 中 |
| **冲突处理** | 本地与云端差异处理策略 | 🟡 中 |
| **增量同步** | 只同步变化的部分 | 🟢 低 |
...
│  步骤 1/3: 选择数据源                                           │
│  ─────────────────────────────────────────────────────────────  │
│  ○ 从 WebDAV 恢复                  [检测到备份: 2026-01-09]     │
│  ○ 从 GitHub 恢复                  [检测到备份: 2026-01-09]     │
│  ● 从本地文件恢复                  [选择文件...]                │
│                                                                 │
│  ─────────────────────────────────────────────────────────────  │
│  步骤 2/3: 选择恢复内容                                         │
...
│                                                                 │
│  恢复预览:                                                      │
│  • 书签: 恢复至 2026-01-09 15:30 (143 个书签)                   │
│  • 历史: 合并 45 条记录，新增 12 条                              │
│  • 视图: 恢复 8 个记录的展开状态                                 │
│                                                                 │
├─────────────────────────────────────────────────────────────────┤
│                        [上一步]    [开始恢复]                    │
...
| 功能 | 存储键 | 说明 | 状态 |
|------|--------|------|------|
| 书签本地备份 | `lastLocalBackupId` | 书签备份 HTML 文件 | 🟢 已实现 |
| 备份历史 HTML | `lastLocalHistoryId_HTML` | 合并模式 HTML 文件 | 🟢 已实现 |
| 备份历史 JSON | `lastLocalHistoryId_JSON` | 合并模式 JSON 文件 | 🟢 已实现 |
| 备份历史 ZIP | `lastLocalHistoryZipId` | ZIP 归档模式 | 🟢 已实现 (2026-01-09) |
