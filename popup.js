...
function toggleConfigPanel(contentElement, headerElement) {
    if (!contentElement || !headerElement) {
        console.error('无法切换配置面板: 元素不存在');
        return;
    }

    console.log('切换配置面板:', contentElement.id);
...
function connectToBackground() {
    try {
        ...
        console.log('尝试连接到后台脚本 (popupConnect)');
    ...
    if (!isBackgroundConnected || !backgroundPort) {
        ...
        setTimeout(() => {
            if (isBackgroundConnected && backgroundPort) {
                ...
                console.log('重新连接失败，无法发送消息');
                // 使用chrome.runtime.sendMessage作为备选方案
                try {
                    chrome.runtime.sendMessage(message, (response) => {
                        if (chrome.runtime.lastError) {
                            console.log('使用runtime.sendMessage发送失败:', chrome.runtime.lastError);
                            if (callback) callback(false, chrome.runtime.lastError);
                        } else {
                            console.log('使用runtime.sendMessage发送成功');
                            if (callback) callback(true, response);
                        }
                    });
                } catch (fallbackError) {
                    console.log('所有消息发送方法都失败:', fallbackError);
        ...
        try {
            ...
            try {
                chrome.runtime.sendMessage(message, (response) => {
                    if (chrome.runtime.lastError) {
                        console.log('备选发送方法失败:', chrome.runtime.lastError);
                        if (callback) callback(false, chrome.runtime.lastError);
                    } else {
                        console.log('使用备选方法发送成功');
                        if (callback) callback(true, response);
                    }
                });
            } catch (fallbackError) {
...
async function initializeWebDAVConfigSection() {
    ...
    if (!configHeader || !configContent) {
        console.error('WebDAV配置区域元素未找到');
        return;
    }

    console.log('初始化WebDAV配置区域');
    ...
    configHeader.addEventListener('click', function(event) {
        ...
        if (event.target.id === 'webDAVToggle' || event.target.closest('.switch')) {
            console.log('点击在开关元素上，不切换面板');
            return;
        }

        console.log('点击WebDAV配置头部，切换面板显示');
        ...
        openDownloadSettings.addEventListener('click', function(e) {
            ...
            chrome.runtime.sendMessage({ action: "openDownloadSettings" }, function(response) {
                if (response && response.success) {
                    console.log('下载设置页面打开成功');
                } else {
                    console.error('打开下载设置页面失败，尝试备用方案');
...
function initScrollToTopButton() {
    ...
    if (일반scrollToTopBtn) {
        ...
        console.log('通用"回到顶部"按钮事件监听器已附加');
        ...
        console.error('未能找到通用"回到顶部"按钮 (ID: scrollToTopBtn)');
    }
    ...
    if (configScrollBtn) {
        ...
        console.log('配置页"回滚至配置页"按钮事件监听器已附加');
    } else {
        console.error('未能找到配置页"回滚至配置页"按钮 (ID: configScrollBtn)');
    }
}
...
async function loadAndDisplayWebDAVConfig() {
    ...
    if (!serverAddressInput || !usernameInput || !passwordInput || !webDAVToggle || !configStatus) {
        console.error('WebDAV配置输入框或开关或状态指示灯元素未找到');
        return;
    }
...
async function loadWebDAVToggleStatus() {
    try {
        ...
        console.error('加载WebDAV开关状态失败:', error);
    }
}
...
function updateSyncHistory(passedLang) { // Added passedLang parameter
    ...
    Promise.all([
            ...
            reversedHistory.forEach((record, index) => {
                ...
                if (record.bookmarkStats) {
                    ...
                    if (cachedRecord && syncHistory.length === 1 && record.time > cachedRecord.time && index === 0) {
                        ...
                        console.log(`HISTORY_ITEM_DISPLAY (using cache for single new item): B:${bookmarkDiff}, F:${folderDiff}`);
                    }
                    // 场景 2: 记录本身包含显式差异 (且未被缓存场景覆盖)
                        ...
                        console.log(`HISTORY_ITEM_DISPLAY (using explicit diff from record): B:${bookmarkDiff}, F:${folderDiff}`);
                    }
                    // 场景 3: 无缓存覆盖、无记录内显式差异，则尝试与列表中的上一条(时间上更早的)记录比较
                        ...
                        if (prevRecordInList && prevRecordInList.bookmarkStats) {
                            ...
                            console.log(`HISTORY_ITEM_DISPLAY (diff from prev in list): B:${bookmarkDiff}, F:${folderDiff}`);
                        }
                    }
                    // 其他情况 (如列表中的第一条记录，但无缓存或不满足缓存条件，且自身无显式差异): diff 保持为 0
                    else {
                        console.log(`HISTORY_ITEM_DISPLAY (no diff calculable, default to 0): B:${bookmarkDiff}, F:${folderDiff}`);
                    }
        ...
        console.error('Error in updateSyncHistory Promise.all:', error);
        const historyList = document.getElementById('syncHistoryList');
        if (historyList) {
    ...
    Promise.all([getLangPromise, getAutoSyncStatePromise])
        .then(([currentLang, isAutoSyncEnabled]) => {
            ...
            if (!bookmarkCountSpan || !changeDescriptionContainer) {
                console.error("Required UI elements (bookmarkCount or change-description-row) not found.");
                return;
            }
            ...
            if (isAutoSyncEnabled) {
                ...
                chrome.runtime.sendMessage({ action: "getBackupStats" }, backupResponse => {
                    if (backupResponse && backupResponse.success && backupResponse.stats) {
                        ...
                        console.warn("Failed to get backup stats for auto-sync display:", backupResponse?.error);
                    }
                });
                ...
                Promise.all([
                    ...
                    console.error('更新书签变动详情失败 (手动模式):', manualError);
        ...
        .catch(initialError => {
            console.error('更新书签计数显示失败 (初始加载语言或同步状态):', initialError);
            const bookmarkCountSpan = document.getElementById('bookmarkCount');
            const changeDescriptionContainer = document.getElementById('change-description-row');
    ...
    chrome.storage.local.set({ autoSync: isAutoSyncEnabled }, () => { // 使用 chrome.storage
        ...
        if (isAutoSyncEnabled) {
            ...
            if (hasBookmarkMoved || hasBookmarkModified) {
                ...
                // syncBookmarks();
                console.warn("syncBookmarks() call in handleAutoSync is commented out as it's not defined here.");
            }
        }
    });
}
...
function handleAutoSyncToggle(event) {
    ...
    console.log('自动备份开关切换为:', isChecked ? '开启' : '关闭');
    ...
    if (!wasChecked && isChecked) { // 只有从 OFF -> ON 才检查
        console.log('[popup.js Log] 主UI: 从手动切换到自动，检查UI是否有变化...'); // <--- Log 1
        ...
        if (changeDescriptionElement) {
            ...
            console.log('[popup.js Log] 主UI: 获取到变化描述文本:', changeText); // <--- Log 2
        ...
        console.log('[popup.js Log] 主UI: 判断UI是否有变化:', uiShowsChanges); // <--- Log 3

        if (uiShowsChanges) {
            console.log('[popup.js Log] 主UI: 检测到变化，准备发送 syncBookmarks(isSwitchToAutoBackup=true)...'); // <--- Log 4
            showStatus('检测到修改，正在为您备份...', 'info', 5000);
            chrome.runtime.sendMessage({
                action: 'syncBookmarks',
                isSwitchToAutoBackup: true
            }, (syncResponse) => {
                console.log('[popup.js Log] 主UI: syncBookmarks(isSwitchToAutoBackup=true) 消息发送完成，响应:', syncResponse); // <--- Log 5
            ...
            console.log('主UI: 未检测到变化，不触发切换备份。');
        }
    }
    // --- 结束新增 ---
    ...
    chrome.runtime.sendMessage({ action: 'toggleAutoSync', enabled: isChecked }, (response) => {
        if (response && response.success) {
            ...
            console.log('后台确认自动备份状态为:', currentAutoSyncState);
                    ...
                    setTimeout(() => {
                        ...
                        if (statsLabels.length > 1) {
                            ...
                            if (syncStatusSection) {
                                console.log('切换到自动备份模式：滚动到"当前数量/结构:"区域');
                            ...
                            console.log('未找到"当前数量/结构:"元素，滚动到页面顶部');
                        }
                    }, 100);
                } else {
                    // 自动备份关闭时，启用按钮并恢复正常外观
            ...
            console.log(`[handleAutoSyncToggle] Successfully toggled. Called updateBookmarkCountDisplay. Mode: ${currentAutoSyncState ? 'Auto' : 'Manual'}`);

            if (wasChecked && !currentAutoSyncState) {
                console.log('切换到手动模式，后台将处理状态更新');
            }
            ...
            updateBookmarkCountDisplay();
            console.log(`[handleAutoSyncToggle] Toggle failed. Called updateBookmarkCountDisplay.`);
        }
    });
}
    ...
    chrome.runtime.sendMessage({
        ...
        if (response && response.success) {
            ...
            if (response.localFileName) {
                ...
                console.log('已保存初始备份记录:', initialBackupRecord);
            }
            ...
            chrome.storage.local.get(['preferredLang'], function(result) {
                const currentLang = result.preferredLang || 'zh_CN';
                console.log('初始化完成后更新历史记录，当前语言:', currentLang);
                updateSyncHistory();
            });
            ...
            setTimeout(() => {
                ...
                if (statsLabels.length > 1) {
                    ...
                    const syncStatusSection = document.getElementById('syncStatus');
                    if (syncStatusSection) {
                        console.log('初始化成功后滚动到"当前数量/结构:"区域');
                        syncStatusSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
                        // 稍微调整位置，确保良好的可视效果
                        window.scrollTo({
                            top: syncStatusSection.offsetTop + 5,
            ...
            chrome.runtime.sendMessage({
                ...
                if (chrome.runtime.lastError) {
                    console.warn("初始化后角标更新消息发送失败:", chrome.runtime.lastError);
                } else if (badgeResponse && badgeResponse.success) {
                    console.log("初始化成功后角标已更新");
                }
            });
        } else {
...
function handleManualUpload() {
    ...
    chrome.runtime.sendMessage({
        ...
        if (response && response.success) {
            ...
            chrome.storage.local.get(['preferredLang'], function(result) {
                const currentLang = result.preferredLang || 'zh_CN';
                console.log('手动备份完成后更新历史记录，当前语言:', currentLang);
                updateSyncHistory();
            });
            ...
            setTimeout(() => {
                ...
                if (statsLabels.length > 1) {
                    ...
                    const syncStatusSection = document.getElementById('syncStatus');
                    if (syncStatusSection) {
                        console.log('手动备份成功后滚动到"当前数量/结构:"区域');
                        syncStatusSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
                        // 稍微调整位置，确保良好的可视效果
                        window.scrollTo({
                            top: syncStatusSection.offsetTop + 5,
    ...
    chrome.runtime.sendMessage({ action: "getSyncHistory" }, (historyResponse) => {
        if (historyResponse && historyResponse.success && historyResponse.syncHistory && historyResponse.syncHistory.length > 0) {
            ...
            chrome.storage.local.set({ cachedRecordAfterClear: latestRecord }, () => {
                console.log('已缓存最后一条记录:', latestRecord);
                // 2. 继续清除实际的历史记录
                proceedToClearActualHistory();
            });
        } else {
            // 没有历史记录可缓存，或者获取失败，则确保清除任何可能存在的旧缓存
            console.log('无历史记录可缓存或获取失败，直接清空并清除旧缓存');
            chrome.storage.local.remove('cachedRecordAfterClear', () => {
                proceedToClearActualHistory();
            });
        }
    ...
    function proceedToClearActualHistory() {
        chrome.runtime.sendMessage({ action: "clearSyncHistory" }, (clearResponse) => {
            if (clearResponse && clearResponse.success) {
                ...
                console.error('清空历史记录失败:', clearResponse?.error);
            }
        });
    }
}
...
async function loadReminderSettings() {
    ...
    try {
        ...
        console.log('手动备份动态提醒设置已加载:', settings);
    } catch (error) {
        console.error('加载手动备份动态提醒设置失败:', error);
...
async function saveReminderSettingsFunc() {
    ...
    try {
        ...
        console.log('手动备份动态提醒设置已保存:', settings);

        // 向后台发送设置更新消息，并添加重置标志
        console.log('发送更新设置消息，将根据新设置重新开始计时');
        ...
        await chrome.runtime.sendMessage({ // 使用 chrome.runtime
            ...
            console.log('设置更新成功，计时器已根据新设置重新开始计时:', response);
        }).catch(error => {
            console.error('更新设置消息发送失败:', error);
        });
        ...
        console.error('保存手动备份动态提醒设置失败:', error);
        return false;
    }
}
    ...
    if (openDialog === 'true') {
        ...
        setTimeout(() => {
            ...
            if (reminderSettingsBtn) {
                console.log('找到手动备份动态提醒设置按钮，自动点击');
                reminderSettingsBtn.click();
            } else {
                console.error('未找到手动备份动态提醒设置按钮');
            }
        }, 500);
    }
}
...
async function initializeLanguageSwitcher() {
    ...
    if (langToggleButton) {
        langToggleButton.addEventListener('click', async () => {
            ...
            try {
                ...
                if (result.initialized === true) {
                    chrome.runtime.sendMessage({
                        ...
                        if (chrome.runtime.lastError) {
                            console.warn("角标更新消息发送失败:", chrome.runtime.lastError);
                        } else if (response && response.success) {
                            console.log("角标已根据新语言更新");
                        }
                    });
                }
            } catch (e) {
                console.warn("Could not save preferred language or update badge.", e);
            }
            ...
            console.log(`已切换语言至: ${currentLang}, 并刷新了历史记录和书签信息`);
        });
    }
}
...
const applyLocalizedContent = async (lang) => { // Added lang parameter
    ...
    if (calibratePathOverlay) {
        console.log('检测到校准路径对话框正在显示，更新其内容');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    chrome.storage.local.get(['autoSync', 'initialized'], function(result) { // 使用 chrome.storage
        ...
        console.log('加载状态: autoSync =', autoSyncEnabled, ', initialized =', initialized);
        ...
        console.log('手动备份选项显示状态:', manualSyncOptions ? manualSyncOptions.style.display : 'element not found');
        ...
        if (message && message.action === "bookmarkChanged") {
            ...
            try {
                ...
                if (changeDescriptionContainer) {
                    ...
                    console.log("返回变化描述内容:", content);
                    sendResponse({
                        success: true,
                        content: content
                    });
                } else {
                    console.warn("未找到变化描述容器元素");
                    sendResponse({
                        success: false,
                        error: "未找到变化描述容器元素",
                        content: "" // 提供空内容
                ...
                console.error("获取变化描述内容失败:", error);
                sendResponse({
                    success: false,
                    error: error.message,
                    content: "" // 提供空内容
    ...
    if (reminderSettingsDialog) {
        reminderSettingsDialog.addEventListener('click', function(event) {
            ...
            if (isOutsideDialog) {
                ...
                console.log('点击对话框外部区域，对话框关闭，计时器恢复');
    ...
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => { // 使用 chrome.runtime
        if (message.action === "showReminderSettings") {
            console.log('收到打开手动备份动态提醒设置对话框消息');
            ...
            if (reminderSettingsDialog) {
                ...
                chrome.runtime.sendMessage({ // 使用 chrome.runtime
                    ...
                    console.error('发送暂停计时器消息失败:', error);
                });
                ...
                console.error('找不到手动备份动态提醒设置对话框元素');
